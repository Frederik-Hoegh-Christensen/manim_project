...
class FirstAbstractionLevel(Scene):
    def construct(self):
        ...
        def push(element):
                # Create a square and text, then group them
                rect = Square(side_length=1, color=WHITE)
                text = Text(element, color=WHITE, font_size=36).move_to(rect.get_center())
                #stack_element = VGroup(rect, text).shift(UP*self.stack_height)
                stack_element = self.expression_group[self.counter]

                
                enclosing_square = SurroundingRectangle(stack_element, color=BLUE, buff=0.1)
                
                target_position = DOWN + LEFT * 2  # Adjust as needed
